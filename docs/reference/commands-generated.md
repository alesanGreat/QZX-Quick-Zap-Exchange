...
> Generated on: 2026-07-29 21:57:02
...
**Parameters:**
- `scan_path`: Path to start the scan from (defaults to current working directory) - Optional (default: `.`)
- `dry_run`: If true, only lists files and space saved without deleting anything (true/false) - Optional (default: `true`)
- `max_depth`: Maximum folder depth to traverse recursively (defaults to 4) - Optional (default: `4`)

**Examples:**
- `qzx cleanDevCaches`
  Scan the current directory for cache folders (dry run)
- `qzx cleanDevCaches . false`
  Scan the current directory and actually delete found cache folders
...
- `kill`: Whether to terminate the process utilizing this port (true/false) - Optional (default: `False`)
- `expected_pid`: Exact PID previously observed on the port; required when kill is true to prevent terminating a different process - Optional

**Examples:**
- `qzx inspectPort 3000`
  Check what is listening on port 3000
- `qzx inspectPort 3000 true 12345 --yolo`
  Terminate only PID 12345 when it is still the observed listener and no restorable filesystem target exists
...
**Examples:**
- `qzx killProcess 1234 --yolo`
  Terminate PID 1234 when no restorable filesystem target exists
- `qzx killProcess 1234 true --dangerously-bypass-approvals-and-sandbox`
  Force terminate PID 1234 without a safety backup
