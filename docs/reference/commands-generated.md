...
> Generated on: 2026-07-29 23:58:08
...
**Parameters:**
- `target_host`: Target SSH host (e.g. deploy@5.161.246.120 or hostname) - Required
- `path`: Path to local project root directory (default: '.') - Optional (default: `.`)
- `target_path`: Target path on remote host (e.g. /var/www/html/) - Required
- `port`: SSH Port (default: 22) - Optional (default: `22`)
- `ssh_key`: Path to SSH Private Key (optional) - Optional
- `health_url`: Health check URL (optional) - Optional
- `restart_cmd`: Remote command to restart service (e.g. nginx -s reload, pm2 restart all) (optional) - Optional
- `skip_build`: If True, skip running local build scripts (default: False) - Optional (default: `False`)
- `dry_run`: If True, only show commands that would be executed without running them (default: True) - Optional (default: `True`)

**Examples:**
- `qzx deployProject --target_host deploy@example.test --target_path /var/www/html/ --port 2237`
