...
class SystemDoctorCommand(CommandBase):
    ...
    def execute(self, quick=False):
            ...
            if isinstance(results["cpu"], dict):
                ...
                if usage > 85:
                    issues.append(
                        (
                            "High CPU usage",
                            f"CPU load is currently at {usage}%",
                            "medium",
                        )
                    )
        ...
        try:
            ...
            if isinstance(results["ram"], dict) and "virtual" in results["ram"]:
                ...
                if percent > 90:
                    issues.append(
                        ("Critical memory usage", f"RAM usage is at {percent}%", "high")
                    )
                elif percent > 75:
                    issues.append(
                        ("High memory usage", f"RAM usage is at {percent}%", "medium")
                    )
        except Exception as e:
                ...
                for part in results["disk"]["partitions"]:
                    ...
                    if use_pct > 90:
                        issues.append(
                            (
                                "Critical disk space",
                                f"Partition '{mount}' is {use_pct}% full",
                                "high",
                            )
                        )
                    elif use_pct > 80:
        ...
        if not quick:
            try:
                ...
                if (
                    isinstance(results["errors"], dict)
                    and results["errors"].get("error_count", 0) > 0
                ):
                    issues.append(
                        (
                            "System errors found",
                            f"Detected recent system error log events",
                            "medium",
                        )
                    )
            except Exception as e:
    ...
    def _check_cpu(self, quick):
        ...
        if psutil:
            ...
            info["cores_physical"] = (
                psutil.cpu_count(logical=False) or info["cores_logical"]
            )
    ...
    def _check_network(self, quick):
        ...
        if psutil:
            ...
            try:
                ...
                net_info["interfaces"].append({"name": "hostname_resolve", "ips": ips})
            except Exception:
                pass
                ...
                try:
                    ...
                    for line in res.stdout.split("\n"):
                        if "STATE" in line:
                            ...
                            break
                    critical_services.append({"name": name, "status": status})
                except Exception:
                    pass
        else:
            # Linux systemd check
    ...
    def _check_ports(self):
        ...
        return {
            "listening": sorted(unique_ports, key=lambda x: x["port"])[:50]
        }  # Cap at 50
                    ...
                    try:
                        with winreg.OpenKey(hkey, subkey, 0, winreg.KEY_READ) as key:
                            ...
                            for i in range(info[1]):
                                ...
                                startup.append(
                                    {"name": name, "command": val, "source": label}
                                )
                    except Exception:
                        pass
            except Exception:
