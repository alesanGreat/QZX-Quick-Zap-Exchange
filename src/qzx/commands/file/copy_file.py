...
class CopyFileCommand(CommandBase):
    ...
    examples = [
        ...
        {
            'command': 'qzx copyFile sourcedir destinationdir -r --force',
            'description': 'Replace an existing destination after creating a safety backup'
        }
    ]

    def validate_safety_backup_target(self, target, values):
        """Reject pointless or ambiguous forced copies before taking a backup."""
        ...
        if not source or not os.path.lexists(source):
            return {
                ...
                "message": (
                    f"Source '{source}' does not exist, so nothing was copied."
                ),
                "details": {
    ...
    def execute(self, source, destination, recursive=None, force=False):
        ...
        try:
            ...
            return {
                ...
                "message": (
                    f"Copy from '{source}' to '{destination}' failed: {e}"
                ),
            }
