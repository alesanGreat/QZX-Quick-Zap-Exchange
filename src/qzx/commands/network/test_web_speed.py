...
class TestWebSpeedCommand(CommandBase):
    ...
    def execute(self, max_seconds="3"):
        ...
        for _ in range(3):
            try:
                ...
                req = urllib.request.Request(
                    self.LATENCY_URL, headers={"User-Agent": "QZX Speed Client"}
                )
        ...
        try:
            req = urllib.request.Request(
                self.TEST_URL, headers={"User-Agent": "QZX Speed Client"}
            )
        ...
        msg += f"- Download Speed: {speed_mbps:.2f} Mbps ({speed_mbs:.2f} MB/s)\n"
        ...
        return {
            ...
            "download_speed_mbs": round(speed_mbs, 2),
            "test_details": {
