...
"""Tests for the token-saving currentDir filesystem summary."""
...
def test_size_adds_recursive_totals_without_detail_lists(
    ...
    result = CurrentDirCommand().invoke(["--size"])
...
def test_legacy_short_path_positional_argument_remains_supported(
    tmp_path,
    monkeypatch,
):
    monkeypatch.chdir(tmp_path)

    result = CurrentDirCommand().invoke(["false"])
...
def test_limit_validation_is_structured(tmp_path, monkeypatch):
    ...
    result = CurrentDirCommand().invoke(["--details", "--limit", "0"])
