"""Privacy and API-contract tests for explainFileWithGemini."""

import json

from qzx.commands.system.explain_file_with_gemini import (
    ExplainFileWithGeminiCommand,
)
...
def test_preview_never_reads_credentials_or_contacts_gemini(tmp_path):
    ...
    command = ExplainFileWithGeminiCommand(
        http_client=None,
...
def test_execution_logic_emits_no_unstructured_stdout(tmp_path, capsys):
    ...
    result = ExplainFileWithGeminiCommand().execute(target)
