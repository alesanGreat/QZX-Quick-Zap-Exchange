...
"""Behavior and real-Git tests for prepareRelease."""
...
from qzx.commands.development.prepare_release import PrepareReleaseCommand
...
def test_preview_reports_plan_without_mutating_npm_project(tmp_path):
    ...
    result = PrepareReleaseCommand().execute(
        bump="minor",
        path=str(tmp_path),
        dry_run=True,
    )
...
def test_ambiguous_manifest_requires_explicit_selection(tmp_path):
    ...
    result = PrepareReleaseCommand().execute(path=str(tmp_path))
...
def test_public_live_preparation_creates_backup_before_writing(
    ...
    result = PrepareReleaseCommand().invoke([
        "--path",
        str(project),
        "--dry-run",
        "false",
