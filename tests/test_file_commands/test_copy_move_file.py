...
@pytest.mark.parametrize("recursive", [False, 0, "-r1", "--recursive2"])
def test_move_rejects_partial_directory_semantics_without_mutation(
    recursive,
    tmp_path,
):
    ...
    result = MoveFileCommand().execute(
        str(source),
        str(destination),
        recursive=recursive,
    )
    ...
    assert result["success"] is False
    assert result["error_code"] == "partial_directory_move_unsupported"
    assert (source / "root.txt").read_text(encoding="utf-8") == "root"
    assert (source / "nested" / "child.txt").read_text(
        encoding="utf-8"
    ) == "child"
    assert not destination.exists()


def test_move_directory_without_recursive_flag_moves_complete_tree(tmp_path):
    source = tmp_path / "source"
    destination = tmp_path / "destination"
    (source / "nested").mkdir(parents=True)
    (source / "nested" / "child.txt").write_text(
    ...
    result = MoveFileCommand().execute(str(source), str(destination))
...
def test_regular_file_copy_is_verified_with_sha256(tmp_path):
    ...
    result = CopyFileCommand().execute(str(source), str(destination))
