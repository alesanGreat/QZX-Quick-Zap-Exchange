...
from qzx.commands.file.extract_zip import ExtractZipCommand


def test_missing_zip_path():
    result = ExtractZipCommand().execute("")
...
def test_nonexistent_zip():
    result = ExtractZipCommand().execute("non_existent_archive.zip")
...
def test_invalid_zip(tmp_path):
    ...
    result = ExtractZipCommand().execute(str(bad_zip))
...
def test_public_overwrite_backs_up_target_before_replacement(
    ...
    result = ExtractZipCommand().invoke(
